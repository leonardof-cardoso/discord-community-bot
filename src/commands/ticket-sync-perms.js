  ...
  async execute(interaction) {
    ...
    async function applyPermissionsToTicket(channel, ticketType, ownerUserId) {
      try {
        ...
        // Aplicar permissões base (everyone bloqueado, dono pode ver/falar)
        ...
        for (const roleId of rolesAllow) {
          ...
          await channel.permissionOverwrites.edit(roleId, {
            ...
            SendMessages: false // Staff só pode ver, não enviar (até assumir)
          });
        }
        
        return true;
      } catch (error) {
        console.error(`[SYNC-PERMS] Erro ao atualizar ${channel.name}:`, error);
        return false;
      }
    }
